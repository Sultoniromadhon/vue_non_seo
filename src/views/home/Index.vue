<template>
    <div class="pb-20 pt-20">
        <div class="container mx-auto grid grid-cols-1 p-3 sm:w-full md:w-5/12">
            <div class=" grid grid-cols-1 bg-sulton-400 rounded shadow-md pr-3 pl-3 pb-5 text-sm">
            ...
            <CategoryHome />
             ...
             <div class="p-2"></div>
            <!-- slider -->
            <div class="grid grid-cols-1 rounded shadow-md p-1 text-sm bg-white w-100">
                <Slider />
            </div>
                            ...
                            <div class="md:flex rounded-xl md:p-0">
                                ...
                                <div class="w-full pt-6 p-5 md:p-3 text-center md:text-left space-y-4">
                                    ...
                                    <div class="font-medium">
                                        <div class="mt-3 text-sulton-500 text-xs">
                                            {{ campaign.user.name }}<i class=" p-1 fas fa-check-circle"></i>
                                        </div>
                                        <div v-if="campaign.sum_donation.length > 0">
                                            <div v-for="donation in campaign.sum_donation" :key="donation">

                                                <div class="relative pt-1">
                                                    <div class="overflow-hidden h-2 mb-4 text-xs flex rounded bg-sulton-200">
                                                        <div :style="{width: percentage(donation.total, campaign.target_donation) + '%'}" class="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-sulton-500"></div>
                                                    </div>
                                                </div>

                                                <p class="text-xs text-sulton-500">
                                                    <span class="font-bold text-sulton-600">Rp. {{ formatPrice(donation.total) }} </span> terkumpul dari
                                        ...
                                        <div v-else>
                                            ...
                                            <p class="text-xs text-sulton-500">
                                                <span class="font-bold text-sulton-600">Rp. 0 </span> terkumpul dari
                                                <span class="font-bold">Rp. {{ formatPrice(campaign.target_donation) }}</span>
                                            </p>
                                        </div>
        ...
        <div class="text-center mt-4 mb-4" v-show="nextExists">
            <a @click="loadMore"
                class="bg-sulton-600 text-white p-2 px-3 rounded-md shadow-md focus:outline-none focus:bg-gray-900 cursor-pointer">LIHAT SEMUA <i class="fa fa-long-arrow-alt-right"></i></a>
        </div>
